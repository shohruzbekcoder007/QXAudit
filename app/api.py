"""
FastAPI — QXAudit (host + document RAG helpers).
...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        version=__version__,
        description=(
            "QXAudit — main host agent + docs_ask tool → document RAG helper (Chroma). "
            "Direct RAG: POST /v1/docs/*. Open WebUI gateway compatible (POST /v1/chat)."
        ),
        docs_url="/docs",
        redoc_url="/redoc",
    )
    ...
    def chat(
        ...
        """Gateway entry: QXAudit host keeps context; documents via docs_ask tool."""
        from agents.hermes_host import get_hermes_host

#!/usr/bin/env bash
# Container entrypoint — QXAudit (host + document RAG helpers)
set -euo pipefail
...
export APP_NAME="${APP_NAME:-QXAudit}"
export QXAUDIT_APP_ROOT="${QXAUDIT_APP_ROOT:-$APP_HOME}"
...
export HERMES_ENABLED_TOOLSETS="${HERMES_ENABLED_TOOLSETS:-docs_bridge}"
...
log "Starting QXAudit"
log "APP_NAME=$APP_NAME HERMES_HOME=$HERMES_HOME RAG_CHROMA_ROOT=$RAG_CHROMA_ROOT LLM_MODEL=${LLM_MODEL:-} RAG_ENABLED=${RAG_ENABLED:-true}"
